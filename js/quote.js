...
function shaq2go() {
    ...
    shaq = { creator: auth.auth.usercode, deContact: [ $("#form-contact-to-company").val(), $("#form-contact-to-name").val(), $("#form-contact-to-email").val(), $("#form-contact-to-phone").val() ], deDate: deDate, dePlace: [ To.address1, To.postcode, To.city, To.country_code, To.country_code], dimension: [ "1", parseFloat($('#form-length').val()), parseFloat($('#form-width').val()), parseFloat($('#form-height').val()), parseFloat($('#form-weight').val()), "no"], id: id, key: key, name: key, notes: "Nothing", puContact: [ $("#form-contact-from-company").val(), $("#form-contact-from-name").val(), $("#form-contact-from-email").val(), $("#form-contact-from-phone").val() ], puDate: puDate, puPlace: [ From.address1, From.postcode, From.city, From.country_code, From.country_code ], reported_at: new Date().toISOString(), source: [ auth.auth.usercode ], stackable: "No", status: "created", target: ['UGO'], type: "auction", visible: "public", valid_from: validFrom.toISOString(), valid_until: validUntil.toISOString(), transport: "LAMBDA", currency: "EUR" };
